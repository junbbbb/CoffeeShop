...
public ArrayList<DtoEmployeeManage> selectList(){
		...
		String whereStatement = "select eid, ename, erank, eindate, etelno, ebday, eemail, eaddress from employee ";
		try {
			...
			while (rs.next()) {
				...
				String wkEname = rs.getString(2); // 컬럼이름쓰던가 번호쓰던가 내맘임
				String wkErank = rs.getString(3);
				String wkEindate = rs.getString(4);
				String wkEtelno = rs.getString(5);
				String wkEbday = rs.getString(6);
				String wkEemail = rs.getString(7);
				String wkEaddress = rs.getString(8);
				
				DtoEmployeeManage dto = new DtoEmployeeManage(wkEid, wkEname, wkErank, wkEindate, wkEtelno, wkEbday, wkEemail, wkEaddress);
				dtoList.add(dto);
			}
